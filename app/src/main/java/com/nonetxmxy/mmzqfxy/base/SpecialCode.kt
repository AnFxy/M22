...
object SpecialCode {
    ...
    fun encode(data: ByteArray): String {
        ...
        var n = 0
        while (i <= end) {
            val d = (data[i].toInt() and 0x0ff shl 16 or (data[i + 1].toInt() and 0x0ff shl 8)
    ...
    private fun decode(c: Char): Int {
        return if (c >= 'A' && c <= 'Z') c.code - 65 else if (c >= 'a' && c <= 'z') c.code - 97 + 26 else if (c >= '0' && c <= '9') c.code - 48 + 26 + 26 else when (c) {
            '+' -> 62
            '/' -> 63
            '=' -> 0
    ...
    fun decode(s: String): ByteArray? {
        ...
        val decodedBytes: ByteArray? = bos?.toByteArray()
        try {
            bos?.close()
            bos = null
    ...
    private fun decode(s: String, os: OutputStream?) {
        ...
        while (true) {
            while (i < len && s[i] <= ' ') i++
            if (i == len) break
            val tri = ((decode(s[i]) shl 18) + (decode(s[i + 1]) shl 12) + (decode(
                s[i + 2]
            ) shl 6)
                    + decode(s[i + 3]))
